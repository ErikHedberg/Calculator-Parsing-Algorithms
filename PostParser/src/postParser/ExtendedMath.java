...
/**
 * Created by Erik Hedberg on 4/13/2016.
 */
public class ExtendedMath {
	...
	public static double sum(double i,double t,Function function) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
	    String fSim = function.expression;
	    fSim = Simplifier.AlgebraicSimplification(fSim);
	    //Simplified Cases
	    if (fSim.matches("1(\\.0)?")) {
	        return t + 1 - i;
	    ...
	    }else if(fSim.matches("k\\^2(\\.0)?") && i==0) {//Square pyramidal numbers
	        return ((t * (t + 1)) * (2 * t + 1)) / 6;
	    }else if(fSim.equals(Number.pat+"^k") && i < t) { //Geometric series
	        ...
	        if(i > t) {
	            for (int k = (int) i; k <= t; k++) {
	                total += function.execute(new Number(Integer.toString(k))).value;
	            }
	        }else{
	            return 0;
	        }
    ...
    public static double prod(double i,double t,Function function) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
        String fSim = function.expression;
        double total = 1;
        if(i > t) {
            for (int k = (int) i; k <= t; k++) {
